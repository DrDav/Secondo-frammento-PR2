...
toList (evaluate(For("val", tuponlybool, Not(Var "val"))));;
...
toList (evaluate (For("n", tuponlynum, Appl(true_if_even, Var "n"))));;
(* Applica true_if_even alla sottotupla tuponlynum[-1:-4] *)
toList (evaluate (For("elem", get_tuple(evaluate (Slice(-1, -4, Etuple(tuponlynum)))), Appl(true_if_even, Var "elem"))));;
(* Restituisce una tupla con i resti della divisione per 2 di ogni elemento *)
toList (evaluate (For("n", tuponlynum, Appl(restoper2, Var "n"))));;
...
(* let y = 10 in let funz = fun x -> x*y in let y = 0 in funz 2   => 20 *)
